...
export function Orders() {
	...
	useEffect(() => {
		...
		setLoading(true);
		Promise.all([listRecipes(session.token), listOrders(session.token)])
	...
	return (
		<div className="page orders-page">
			...
			{!session ? (
				<p className="hint">請先登入才能查看與使用點菜功能（只有家人看得到）。</p>
			) : (
				<>
								...
								{category &&
									pageResults.map((recipe) => (
										<RecipeCard key={recipe.id} recipe={recipe} onViewRecipe={setViewing}>
											<button type="button" disabled={addingId === recipe.id} onClick={() => handleAdd(recipe)}>
