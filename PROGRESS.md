...
| 前端登入/導覽框架 + UI 主題 | ✅ 已完成 |
| 佈告欄 / 食譜庫 / 點菜頁面 | ✅ 已實作（讀取 + 新增）。食譜庫已改版：菜色圖固定用插畫（移除菜色照片上傳）；新增「食譜圖」功能——登入者可對每道菜上傳手寫食譜照片（卡片角落「＋食譜」），有食譜圖的卡片顯示「📖 食譜」按鈕點開彈窗看圖（`recipeUrl` 欄位、`POST /api/recipes/recipe-image`）。新增菜色只填菜名＋分類（＋可選食譜圖），菜色圖之後補插畫 |
