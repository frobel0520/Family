...
## 照片補齊進度（Wikimedia Commons 開放授權圖，已全分類掃過一輪）

**62/160 道有照片**。做法：每道菜用英文關鍵字搜 Wikimedia Commons → 下載 → 裁成 800x800 正方形 → **逐張人工目視確認正確才套用**（自動配對錯誤率約一半，出現過把「三杯雞」配到廟宇柱子、「煎牛小排」配到人體解剖圖、「梅子綠茶」配到 Second Life 遊戲截圖這種離譜結果，所以人工確認不能省）。圖片存 `images/recipes/{id}.jpg`，出處/作者/授權記在 `recipes.json` 的 `photoCredit` 欄位，前端顯示小字出處連結（CC 授權要求標示）。

各分類命中率：麵食 12/18、快炒／主菜 13/34、點心 13/29、飯類 8/22、湯品 7/23、炸物／烤物 5/10、滷味 3/10、飲料 1/14（手搖飲在 Commons 幾乎沒有可用圖）。沒圖的 98 道多半是台式家常菜（豬血湯、鹽水雞、養樂多燉肉這類），開放圖庫找不到、等家人自己拍最實際。

## 下一步（依優先順序）

1. 佈告欄「編輯/刪除自己的留言」——Worker 目前只有新增（POST），`board.json` 的 `updatedAt` 欄位已經預留但沒用到
2. 剩下 98 道沒照片的菜等家人上傳實拍（上傳會自動蓋掉預設圖示；有 photoCredit 的圖被家人的實拍蓋掉也沒關係）
